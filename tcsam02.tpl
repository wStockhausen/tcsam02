...
//-2020-03-02:  1. Added ability to specify cubic splines in MPI..
//              2. Removed all param_init_(bounded_)vector_vector objects in favor of
...
//                  data are adjusted now when doRetro is "on" and yRetro=0, as well as
...
//-2020-07-06:  1. Revised retrospective calculations so likelihood for devs include
//                   all values (estimated AND non-estimated). This is done so that
//                   the likelihood penalty on summation-to-zero is minimized when
//                   starting with converged model values from a non-retrospective model run.
...
//-2020-11-03:  1. Changed jittering so one can now jitter when using a pin file.
//                  The pin file values are first assigned, then "centered" jittering
//                  is done based on these values (rather than on the values defined
//                  in the MPI file). Makes it a bit easier to test convergence from
...
//-2020-12-01:  1. Revised size frequency data input to specify DM parameter index to use by
//                   factor combination and year.
//-2020-12-03:  1. Finished revision to size frequency data input to specify DM parameter index to use by
//                   factor combination and year.
...
//              2. Changed parameter names from pIniRec, pInitNatZ to pLnBaseInitN, pvLnInitNatZ
//                   to better reflect parameter values.
//-2022-04-14:  1. Revised pvLnInitNatZ so its first element NO LONGER corresponds to
//                   the base x,m,s,z (and thus was previously required to be 0 and not estimated).
//-2022-04-17:  1. Added several model options for estimating initial N's
//              2. Revised pvLnInitNatZ so parameters are EITHER on ln-scale or logistic scale.
//              3. If on ln-scale, uses n-1 parameters (n: number of initial non-zero classes)
//              4. If on logistic scale, uses n parameters (and priors should be assigned)
...
//-2022-08-05: 1. Revised how initial N's are calculated when parameters are on logistic scale so 
//                  that normalization is calculated over ALL population components (as intended), 
//                  not over individual components. Individual population components 
//                  were incorrectly scaled to "total abundance" (exp(pLnBaseInitN)). Due to the length
//                  of the time series (starting in 1928), this didn't have much effect on 
//                  population estimates in final years.
//             2. Removed commandline option "-optInitialNs val" because options for initial Ns
//                  cannot be specified on the commandline any more.
//             3. Updated ModelOptions VERSION to "2022.08.05".
//-2022-08-06: 1. Revised how initial N's parameters are initialized when estimated, depending on whether
//                  initialization is through MPI/pin file values or equilibrium calculations.
//-2022-08-08: 1. Finished revision of how initial N parameters are initialized when estimated, depending on whether
//                  initialization is through MPI/pin file values or equilibrium calculations.
//             2. Removed sum-to-1 normalization when initializing InitNatZ parameters when opts3_InitNatZ=2
//                  (sum-to-1 penalty imposed) to start from identical conditions when using a pin file for
//                  initialization. Because a likelihood penalty was used to impose the sum-to-1 constraint,
//                  the resulting parameter estimates did not necessarily sum-to-1 identically. As a consequence, 
...
//    exit(1);
 ...
 END_CALCS
    ...
    !!PRINT2B1("")
    !!PRINT2B1("Extracting parameter info: ")
 ...
 END_CALCS    
    ...
    int npLnBaseInitN; ivector phsLnBaseInitN; vector lbLnBaseInitN; vector ubLnBaseInitN;
    !!tcsam::setParameterInfo(ptrMPI->ptrINs->pLnBaseInitN,npLnBaseInitN,lbLnBaseInitN,ubLnBaseInitN,phsLnBaseInitN,rpt::echo);
    
    int npLnInitNatZ; int nptLnInitNatZ;
    ivector mniLnInitNatZ; ivector mxiLnInitNatZ; imatrix idxsLnInitNatZ;
    vector lbLnInitNatZ; vector ubLnInitNatZ; ivector phsLnInitNatZ;
    !!tcsam::setParameterInfo(ptrMPI->ptrINs->pvLnInitNatZ,npLnInitNatZ,nptLnInitNatZ,mniLnInitNatZ,mxiLnInitNatZ,idxsLnInitNatZ,lbLnInitNatZ,ubLnInitNatZ,phsLnInitNatZ,rpt::echo);
 LOCAL_CALCS
    if (ptrMPI->ptrINs->pvLnInitNatZ->getSize()>0){
        rpt::echo<<"phsLnInitNatZ = "<<phsLnInitNatZ<<endl;
        rpt::echo<<"idxsLnInitNatZ"<<endl<<idxsLnInitNatZ<<endl;
        for (int i=1;i<=npLnInitNatZ;i++) rpt::echo<<"fwdIndices["<<i<<"] = "<<(*(ptrMPI->ptrINs->pvLnInitNatZ))[i]->getFwdIndices()<<endl;
    }
 END_CALCS
 LOCAL_CALCS    
    if (mseOpModMode) {
 ...
 END_CALCS    
    ...
    //maturity parameters    
    int npLgtPrM2M; int nptLgtPrM2M;
    ivector mniLgtPrM2M; ivector mxiLgtPrM2M; imatrix idxsLgtPrM2M;
    vector lbLgtPrM2M; vector ubLgtPrM2M; ivector phsLgtPrM2M;
    !!tcsam::setParameterInfo(ptrMPI->ptrM2M->pvLgtPrM2M,npLgtPrM2M,nptLgtPrM2M,mniLgtPrM2M,mxiLgtPrM2M,idxsLgtPrM2M,lbLgtPrM2M,ubLgtPrM2M,phsLgtPrM2M,rpt::echo);
 LOCAL_CALCS
    if (ptrMPI->ptrM2M->pvLgtPrM2M->getSize()>0){
        rpt::echo<<"idxsLgtPrM2M"<<endl<<idxsLgtPrM2M<<endl;
        for (int i=1;i<=npLgtPrM2M;i++) rpt::echo<<"fwdIndices["<<i<<"] = "<<(*(ptrMPI->ptrM2M->pvLgtPrM2M))[i]->getFwdIndices()<<endl;
    }
 END_CALCS
 LOCAL_CALCS    
    if (mseOpModMode) {
 ...
 END_CALCS    
    ...
    int npNPSel; int nptNPSel;
    ivector mniNPSel; ivector mxiNPSel; imatrix idxsNPSel;
    vector lbNPSel; vector ubNPSel; ivector phsNPSel;
    !!tcsam::setParameterInfo(ptrMPI->ptrSel->pvNPSel,npNPSel,nptNPSel,mniNPSel,mxiNPSel,idxsNPSel,lbNPSel,ubNPSel,phsNPSel,rpt::echo);
 LOCAL_CALCS
    if (ptrMPI->ptrSel->pvNPSel->getSize()){
        rpt::echo<<"idxsNPSel"<<endl<<idxsNPSel<<endl;
        for (int i=1;i<=npNPSel;i++) rpt::echo<<"fwdIndices["<<i<<"] = "<<(*(ptrMPI->ptrSel->pvNPSel))[i]->getFwdIndices()<<endl;
    }
 END_CALCS
    
    int npCubSplns; int nptCubSplns;
    ivector mniCubSplns; ivector mxiCubSplns; imatrix idxsCubSplns;
    vector lbCubSplns; vector ubCubSplns; ivector phsCubSplns;
    !!tcsam::setParameterInfo(ptrMPI->ptrSel->pvCubSplns,npCubSplns,nptCubSplns,mniCubSplns,mxiCubSplns,idxsCubSplns,lbCubSplns,ubCubSplns,phsCubSplns,rpt::echo);
 LOCAL_CALCS
    if (ptrMPI->ptrSel->pvCubSplns->getSize()){
        rpt::echo<<"idxsCubSplns"<<endl<<idxsCubSplns<<endl;
        for (int i=1;i<=npCubSplns;i++) rpt::echo<<"fwdIndices["<<i<<"] = "<<(*(ptrMPI->ptrSel->pvCubSplns))[i]->getFwdIndices()<<endl;
    }
 END_CALCS    
 LOCAL_CALCS
    if (mseOpModMode) {
 ...
 END_CALCS    
    ...
    !!if ( mseOpModMode) {phsMSE_LnC =  1;}
    !!PRINT2B1("Finished extracting parameter info.")
    !!PRINT2B1("")
...
PARAMETER_SECTION
 ...
 END_CALCS
    ...
    //initial N-at-Z parameters
    init_bounded_number_vector pLnBaseInitN(1,npLnBaseInitN,lbLnBaseInitN,ubLnBaseInitN,phsLnBaseInitN);    //base ln-scale initial N-at-Z
    !!PRINT2B2("pLnBaseInitN = ",pLnBaseInitN)
 LOCAL_CALCS
    if (ptrMPI->ptrINs->pvLnInitNatZ->getSize()>0){
        PRINT2B2("npLnInitNatZ  = ",npLnInitNatZ)
        PRINT2B2("nptLnInitNatZ = ",nptLnInitNatZ)
        PRINT2B2("lbLnInitNatZ  = ",lbLnInitNatZ)
        PRINT2B2("ubLnInitNatZ  = ",ubLnInitNatZ)
        PRINT2B2("phsLnInitNatZ = ",phsLnInitNatZ)
    }
 END_CALCS
    init_bounded_number_vector pvLnInitNatZ(1,nptLnInitNatZ,lbLnInitNatZ,ubLnInitNatZ,phsLnInitNatZ);//ln-scale N-at-Z offsets
    !!PRINTVV("pvLnInitNatZ",LnInitNatZ)
    matrix matLnInitNatZ(1,npLnInitNatZ,mniLnInitNatZ,mxiLnInitNatZ);
    ...
    !!PRINT2B2("pGrBeta = ",pGrBeta)
    !!PRINT2B1("got past growth parameters")
    
    //maturity parameters
    init_bounded_number_vector pvLgtPrM2M(1,nptLgtPrM2M,lbLgtPrM2M,ubLgtPrM2M,phsLgtPrM2M);//logit-scale maturity ogive parameters
 LOCAL_CALCS
    if (ptrMPI->ptrM2M->pvLgtPrM2M->getSize()>0){
       PRINT2B2("npLgtPrM2M  = ",npLgtPrM2M)
       PRINT2B2("nptLgtPrM2M  = ",nptLgtPrM2M)
 ...
 END_CALCS
    ...
    matrix matLgtPrM2M(1,npLgtPrM2M,mniLgtPrM2M,mxiLgtPrM2M);
 ...
 LOCAL_CALCS
    if (ptrMPI->ptrSel->pvNPSel->getSize()>0){
        ...
        PRINT2B2("nptNPSel = ",nptNPSel)
        PRINT2B2("lbNPSel  = ",lbNPSel)
        PRINT2B2("ubNPSel  = ",ubNPSel)
 ...
 LOCAL_CALCS
    if (ptrMPI->ptrFsh->pDevsLnC->getSize()>0){
...
//---------PRELIMINARY_CALCS: ALL MODEL RUNS------------------------------------    
    ...
    //Apply ModelOptions for initial abundance at size
    PRINT2B1("")
    PRINT2B1("----Applying ModelOptions to initial abundance at size----")
    if (ptrMOs->opt1_InitNatZ==0){
        //do nothing: build up pop from 0 using recruitment during pop dynamics calculations
        PRINT2B1("--InitNatZ: building population from 0 using recruitment!")
    } else if (ptrMOs->opt1_InitNatZ==1){
        //nothing to do: calculated from unfished equilibrium assumptions during pop dynamics calculations
        PRINT2B1("--InitNatZ: no estimation! using unfished equilibrium assumptions")
    } else if (ptrMOs->opt1_InitNatZ>1){
        PRINT2B1("--InitNatZ: estimating InitNatZ parameters!")
        if (ptrMOs->opt2_InitNatZ==1){
            //using input values from MPI or pin file
            PRINT2B1("--InitNatZ: using input values from MPI or pin file (no need to do anything)")
//            if (ptrMOs->opt1_InitNatZ==2){
//                PRINT2B1("--InitNatZ: ptrMOs->opt1_InitNatZ==2 so no need to do anything on initialization.")
//            }
//            if (ptrMOs->opt1_InitNatZ==3){//scale parameter values to make sure initial abundance fractions sum to 1 over all PCs
//                PRINT2B1("--InitNatZ: ptrMOs->opt1_InitNatZ==3, so scale parameter values so initial abundance fractions sum to 1 over all PCs.")
//                InitialNatZInfo* ptrINs = ptrMPI->ptrINs;
//
//                PRINT2B1("Calculating normalization factor")
//                dvariable norm_fac = 0.0; //normalization factor: sum over 1/(1+exp(-vLnInitNatZ))
//                int k;
//                for (int pc=1;pc<=ptrINs->nPCs;pc++){
//                    ivector pids = ptrINs->getPCIDs(pc);
//                    k=ptrINs->nIVs+1;//index to first parameter variable column in ParameterComnbinations
//                    k++;             //skip first variable (pLnBaseInitVal) this pass
//
//                    rpt::echo<<"--pc = "<<pc<<tb<<"k = "<<k<<tb<<"pids[k] = "<<pids[k]<<endl;
//                    BoundedVectorInfo* pBVI = (*ptrINs->pvLnInitNatZ)[pids[k]];
//                    rpt::echo<<"--matLnInitNatZ(pids[k]) = "<<matLnInitNatZ(pids[k])<<endl;
//                    dvar_vector vLnInitNatZ = pBVI->calcArithScaleVals(matLnInitNatZ(pids[k]));
//                    rpt::echo<<"--vLnInitNatZ            = "<<vLnInitNatZ<<endl;
//                    dvar_vector prbs = 1.0/(1.0+mfexp(-vLnInitNatZ));//--unnormalized fractions
//                    norm_fac += sum(prbs);                           //--add sum to normalization factors
//                }//--pc
//                //recalculate initial parameter values
//                PRINT2B1("Recalculating parameter values after initialization")
//                for (int pc=1;pc<=ptrINs->nPCs;pc++){
//                    ivector pids = ptrINs->getPCIDs(pc);
//                    k=ptrINs->nIVs+1;//first parameter variable column in ParameterCombinations
//                    k++;//skip ln-scale total abundance
//
//                    rpt::echo<<"--pc = "<<pc<<tb<<"k = "<<k<<tb<<"pids = "<<pids<<endl;
//                    BoundedVectorInfo* pBVI = (*ptrINs->pvLnInitNatZ)[pids[k]];
//                    rpt::echo<<"--matLnInitNatZ(pids[k]) = "<<matLnInitNatZ(pids[k])<<endl;
//                    dvar_vector vLnInitNatZ = pBVI->calcArithScaleVals(matLnInitNatZ(pids[k]));//--logistic-scale parameter values for this PC
//                    rpt::echo<<"--vLnInitNatZ            = "<<vLnInitNatZ<<endl;
//                    dvar_vector prbs        = 1.0/(1.0+mfexp(-vLnInitNatZ));                   //--arithmetic-scale values
//                    prbs                    = prbs/norm_fac;                                   //--normalize to sum to 1
//                    vLnInitNatZ             = log(prbs)-log(1.0-prbs);                         //--transformed back to logistic scale
//                    rpt::echo<<"--vLnInitNatZ            = "<<vLnInitNatZ<<endl;
//                    pBVI->setInitValsFromParamVals(vLnInitNatZ);//set initial parameter values in BVI
//                    rpt::echo<<"--pBVI->getInitVals()    = "<<pBVI->getInitVals()<<endl;
//                    matLnInitNatZ(pids[k]) = vLnInitNatZ;       //set initial parameter values in matrix
//                    rpt::echo<<"--matLnInitNatZ(pids[k]) = "<<matLnInitNatZ(pids[k])<<endl;
//                }//--pc
//                PRINT2B1("Copying values to param_init_bounded_number_vector")
//                int ctr = 0;
//                for (int i=1;i<=npLnInitNatZ;i++){
//                    rpt::echo<<"--i: "<<i<<tb<<matLnInitNatZ(i)<<endl;
//                    int ctr_start = ctr+1;
//                    for (int j=matLnInitNatZ(i).indexmin();j<=matLnInitNatZ(i).indexmax();j++) 
//                        pvLnInitNatZ[++ctr] = matLnInitNatZ(i,j);//set initial values for parameters
//                    int ctr_end = ctr;
//                    rpt::echo<<"--pvLnInitNatZ: "; for (int j=ctr_start;j<=ctr_end;j++) {rpt::echo<<pvLnInitNatZ(j)<<" ";} rpt::echo<<endl;
//                }
//                PRINT2B1("end")
//            }
        }//--ptrMOs->opt2_InitNatZ==1
        if (ptrMOs->opt2_InitNatZ==2){
            //initialize parameter values for initial Ns using equilibrium calculations (ignore MPI/pin values)
            PRINT2B1("--InitNatZ: initializing parameter values using equilibrium calculations (ignoring MPI/pin values)")
            if (usePin) {
                rpt::echo<<endl<<endl<<"#########################"<<endl;
                rpt::echo<<"Using pin values, but initializing initNatZ parameter values from equilibrium calculations."<<endl;
                rpt::echo<<"#########################"<<endl<<endl<<endl;
                std::cout<<endl<<endl<<"#########################"<<endl;
                std::cout<<"Using pin values, but initializing initNatZ parameter values from equilibrium calculations."<<endl;
                std::cout<<"#########################"<<endl<<endl<<endl;
            }
            //re-initialize pLnBaseInitN and pvLnInitNatZ based on equilibrium calculations
            int dbgLevel = 0;
            PRINT2B1(" ")
            PRINT2B1("--re-initialize pLnBaseInitN and pvLnInitNatZ based on equilibrium calculations")
            ...
            calcEqNatZF100(avgRec, mnYr, 0, rpt::echo);//--calculate n_xmsz
            ...
            if (nPCs!=npLnInitNatZ){
                PRINT2B1("--ERROR INITIALIZING NUMBER AT SIZE: nPCs!=npLnInitNatZ--")
                PRINT2B2("nPCs = ",nPCs)
                PRINT2B2("npLnInitNatZ = ",npLnInitNatZ)
                exit(-1);
            }
            if (nPTs!=nptLnInitNatZ){
                PRINT2B1("--ERROR INITIALIZING NUMBER AT SIZE: nPTs!=nptLnInitNatZ--")
                PRINT2B2("nPTs = ",nPTs)
                PRINT2B2("nptLnInitNatZ = ",nptLnInitNatZ)
                exit(-1);
            }
            ...
            for (int x=1;x<=nSXs;x++){
                for (int m=1;m<=nMSs;m++){
                    for (int s=1;s<=nSCs;s++){
                        if (lb(x,m,s)>0) {
                            iPC++; lbv(iPC)=lb(x,m,s); ubv(iPC)=ub(x,m,s);
                        }
                    }
                }
            }
            ...
            if (sum(wts::abs(mxi-mxiLnInitNatZ))){
                PRINT2B1("--ERROR INITIALIZING NUMBER AT SIZE: indices mismatch--")
                PRINT2B2("revised mxi = ",mxi)
                PRINT2B2("mniLnInitNatZ = ",mniLnInitNatZ)
                PRINT2B2("mxiLnInitNatZ = ",mxiLnInitNatZ)
                exit(-1);
            }

            //update pLnBaseInitN and pvLnInitNatZ
            //TODO: pretty sure the old way ONLY WORKS when x,m,s sequence here agrees with order in MPI!!
            if (ptrMOs->opt1_InitNatZ==2){
                ...
                int ip = 0;//--index into pvLnInitNatZ
                for (int pc=1;pc<=ptrINs->nPCs;pc++){
                    ivector pids = ptrINs->getPCIDs(pc);
                    int x = pids[1];
                    int m = pids[2];
                    int s = pids[3];
                    int k=ptrINs->nIVs+1;//first parameter variable column in ParameterComnbinations
                    k++;                 //skip first parameter (pLnBaseInitN)
                    ...
                    if (debug>dbgCalcProcs){
                        ...
                        cout<<"vLnInitNatZ = "<<matLnInitNatZ(pids[k])<<endl;//TODO: is index pids[k] correct here??
                    ...
                    if (pc==1) {
                        pLnBaseInitN[1] = log(n_xmsz(x,m,s,idxf(imn)-1));//idxf(imn)-1 is index of base size class
                        if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<idxf(imn)-1<<tb<<0<<tb<<pLnBaseInitN[1]<<endl;
                    }
                    for (int i=imn;i<=imx;i++){
                        matLnInitNatZ(pids[k])(i) = log(n_xmsz(x,m,s,idxf(i)))-pLnBaseInitN[1];
                        pvLnInitNatZ(++ip)        = matLnInitNatZ(pids[k++])(i);
                        if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<idxf(i)<<tb<<i<<tb<<matLnInitNatZ(pids[k])(i)<<endl;
                    }
                    pBVI->setInitValsFromParamVals(matLnInitNatZ(pids[k]));//set initial parameter values in BVI
                }//--pc
            } //--ptrMOs->opt1_InitNatZ==2
            else {
                //--SCALED version: pLnBaseInitN is ln-scale total abundance
                dvariable totN = sum(n_xmsz);
                pLnBaseInitN[1] = log(totN);//--only one element to update
                //--pvLnInitNatZ are logistic-scale parameters for non-zero size classes
                InitialNatZInfo* ptrINs = ptrMPI->ptrINs;
                int ip = 0;       //--index into pvLnInitNatZ
                double totP = 0.0;//--check on sum-to-1
                for (int pc=1;pc<=ptrINs->nPCs;pc++){
                    ivector pids = ptrINs->getPCIDs(pc);
                    int x = pids[1];
                    int m = pids[2];
                    ...
                    if (debug>dbgCalcProcs) cout<<"x"<<tb<<"m"<<tb<<"s"<<tb<<"z"<<tb<<"i"<<tb<<"vLnInitNatZ(i)"<<endl;
                    for (int i=imn;i<=imx;i++){
                        dvariable prb = n_xmsz(x,m,s,idxf(i))/totN;
                        totP         += value(prb);
                        matLnInitNatZ(pids[k])(i) = log(prb)-log(1.0-prb);
                        pvLnInitNatZ(++ip)        = matLnInitNatZ(pids[k])(i);
                        if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<idxf(i)<<tb<<i<<tb<<matLnInitNatZ(pids[k])(i)<<endl;
                    }
                    pBVI->setInitValsFromParamVals(matLnInitNatZ(pids[k]));//set initial parameter values in BVI
                ...
                rpt::echo<<"totN = "<<totN<<tb<<"totP = "<<totP<<tb<<endl;
            }//--ptrMOs->opt1_InitNatZ==2

            rpt::echo<<"BEFORE updating parameter values"<<endl;
            rpt::echo<<"pLnBaseInitN = "<<pLnBaseInitN<<endl;;
            rpt::echo<<"matLnInitNatZ = "<<endl;
            for (int i=1;i<=npLnInitNatZ;i++) rpt::echo<<i<<" : "<<matLnInitNatZ(i)<<endl;

            //--update parameter info for ptrMPI->ptrINs->pLnBaseInitN and ptrMPI->ptrINs->pvLnInitNatZ
            ptrMPI->ptrINs->pLnBaseInitN->setInitValsFromParamVals(pLnBaseInitN);
            int ctr = 1;
            for (int i=1;i<=npLnInitNatZ;i++) {
                BoundedVectorInfo* ptrI = static_cast<BoundedVectorInfo*>((*ptrMPI->ptrINs->pvLnInitNatZ)[i]);
                dvector pnvls(1,mxi(i));                       //initial values on parameter scale from pin file
                for (int j=1;j<=mxi(i);j++) pnvls(j) = value(pvLnInitNatZ(ctr++));
                ...
                //rpt::echo<<tb<<"final param inits: "; for (int j=1;j<=mxi(i);j++) rpt::echo<<pvLnInitNatZ(ctr++)<<tb; rpt::echo<<endl;//final initial values on parameter scale from parameter info
            }
            //--set values in matLnInitNatZ
            tcsam::setBoundedVectorVector(matLnInitNatZ,pvLnInitNatZ,ptrMPI->ptrINs->pvLnInitNatZ,0,rpt::echo);

            //--
            rpt::echo<<"AFTER updating parameter values"<<endl;
            rpt::echo<<"*ptrMPI->ptrINs->pLnBaseInitN = "<<endl<<(*ptrMPI->ptrINs->pLnBaseInitN)<<endl;;
            rpt::echo<<"pLnBaseInitN = "<<pLnBaseInitN<<endl;;
            rpt::echo<<"ptrMPI->ptrINs->pvLnInitNatZ = "<<endl<<(*ptrMPI->ptrINs->pvLnInitNatZ)<<endl;;
            rpt::echo<<"matLnInitNatZ = "<<endl;
            for (int i=1;i<=npLnInitNatZ;i++) rpt::echo<<i<<" : "<<matLnInitNatZ(i)<<endl;
            ...
            rpt::echo<<"total initial abundance (original) = "<<sum(n_xmsz)<<endl;

            PRINT2B1("--testing estimateInitialNatZ")
            estimateInitialNatZ(dbgLevel,rpt::echo);
            PRINT2B1("--Estimated n_xmsz from parameter values")
            for (int x=1;x<=nSXs;x++){
                for (int m=1;m<=nMSs;m++){
                    for (int s=1;s<=nSCs;s++){
                        rpt::echo<<tb<<x<<tb<<m<<tb<<s<<tb<<n_xmsz(x,m,s)<<endl;
...
//---------PRELIMINARY_CALCS: NON-MSE MODEL RUNS ONLY---------------------------    
        ...
        setAllVectorVectors(tcsam::dbgAll,rpt::echo);
...

        {
            ...
            os2.precision(12);
            writeParameters(os2,0,1,0);//only parameters that will be active (i.e., phase>0)
            os2.close();
...
//-----------PRELIMINARY_CALCS: "ORDINARY" MODEL RUNS ONLY---------------------- 
            ...
            if (doOFL&&debugOFL){
                ...
                echoOFL1.precision(12);
                echoOFL1<<"----Testing calcOFL()"<<endl;
                calcOFL(mxYr+1-yRetro,debugOFL,echoOFL1);//updates ptrOFLResults
                ptrOFLResults->writeCSVHeader(echoOFL1); echoOFL1<<endl;
                ptrOFLResults->writeToCSV(echoOFL1);     echoOFL1<<endl;
                echoOFL1<<"----Finished testing calcOFL()!"<<endl;
                echoOFL1.close();
                PRINT2B1("Finished testing OFL calculations 1")
                PRINT2B1("Testing OFL calculations 2")
                ofstream echoOFL2; echoOFL2.open("calcOFL.init2.txt", ios::trunc);
                echoOFL2.precision(12);
                echoOFL2<<"----Testing calcOFL()"<<endl;
                calcOFL(mxYr+1-yRetro,debugOFL,echoOFL2);//updates ptrOFLResults
                ptrOFLResults->writeCSVHeader(echoOFL2); echoOFL2<<endl;
                ptrOFLResults->writeToCSV(echoOFL2);     echoOFL2<<endl;
                echoOFL2<<"----Finished testing calcOFL()!"<<endl;
                echoOFL2.close();
                PRINT2B1("Finished testing OFL calculations 2")
                if (doProjections){
                    PRINT2B1("Testing multi-year projections 1")
                    ofstream prjT; prjT.open("calcProjections.init.txt", ios::trunc);
                    calcMultiYearProjections(mxYr+1-yRetro, ptrMOs->ptrProjOpts, debugOFL, prjT);
                    prjT.close();
...
FUNCTION void initAltPopDyMod(int debug, ostream& cout)
    ...
    setAllVectorVectors(debug,cout);//set devs vectors
    ...
    if (ptrMOs->opt1_InitNatZ==0){
        ...
        n_yxmsz(mnYr) = n_xmsz;
    } else if (ptrMOs->opt1_InitNatZ<4){
        //calculate initial n-at-z from estimated parameters
        estimateInitialNatZ(debug,cout);
        n_yxmsz(mnYr) = n_xmsz;
    } else {
        cout<<"Unrecognized option for initial n-at-z: "<<ptrMOs->opt1_InitNatZ<<endl;
...
FUNCTION void initPopDyMod(int debug, ostream& cout)
    ...
    setAllVectorVectors(debug,cout);//set vector_vectors
...
FUNCTION void estimateInitialNatZ(int debug, ostream& cout)
    ...
    if (ptrMOs->opt1_InitNatZ==2){
        //UNSCALED version (ln scale)
        estimateInitialNatZ_LnScale(debug, cout);
    } else {
        //SCALED version (logistic scale)
        estimateInitialNatZ_LogisticScale(debug, cout);
    }
    if (debug>dbgCalcProcs){
        cout<<"n_xmsz: "<<endl;
        for (int x=1;x<=nSXs;x++){
...
//-------------------------------------------------------------------------------------
//calculate initial numbers-at-size from estimated log-scale parameters
//UNSCALED version (ln scale)
FUNCTION void estimateInitialNatZ_LnScale(int debug, ostream& cout)
    if (debug>=dbgPopDy) cout<<"starting void estimateInitialNatZ_LnScale()"<<endl;
    InitialNatZInfo* ptrINs = ptrMPI->ptrINs;
    
    n_xmsz.initialize();//equilibrium n-at-z
    
    dvar_vector ptLnBaseInitN  = ptrINs->pLnBaseInitN->calcArithScaleVals(pLnBaseInitN);
    ...
    for (int pc=1;pc<=ptrINs->nPCs;pc++){
        ...
        dvariable lnInitN = ptLnBaseInitN(pids[k++]);
        ...
        if (debug>dbgCalcProcs){
            cout<<"pc = "<<pc<<". x = "<<x<<". m = "<<m<<". s = "<<s<<endl;
            cout<<"imnz = "<<imn<<" imxz = "<<imx<<endl;
            cout<<"vLnInitNatZ = "<<vLnInitNatZ<<endl;
            cout<<"fwd indices = "<<idxf<<endl;
            ivector idxr = pBVI->getRevIndices();
            cout<<"rev indices min, max = "<<idxr.indexmin()<<cc<<idxr.indexmax()<<endl;
            cout<<"rev indices = "<<idxr<<endl;
        ...
        if (pc==1) {
            z0 = idxf(imn)-1;
            n_xmsz(1,1,1,z0) = mfexp(lnInitN);//z= idxf(imn)-1 is base size class
            if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<z0<<tb<<0<<tb<<lnInitN<<endl;
        }
        for (int i=imn;i<=imx;i++){
            n_xmsz(x,m,s,idxf(i)) = mfexp(lnInitN+vLnInitNatZ(i));
            if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<idxf(i)<<tb<<i<<tb<<vLnInitNatZ(i)<<endl;
        }
    }//--pc
    if (debug>=dbgPopDy) cout<<"finished void estimateInitialNatZ_LnScale()"<<endl;

//-------------------------------------------------------------------------------------
//calculate initial numbers-at-size from logistic-scale estimated parameters
//SCALED version (logistic scale)
FUNCTION void estimateInitialNatZ_LogisticScale(int debug, ostream&cout)
    if (debug>=dbgPopDy) cout<<"starting void estimateInitialNatZ_LogisticScale()"<<endl;
    InitialNatZInfo* ptrINs = ptrMPI->ptrINs;
    ...
    dvariable norm_fac = 1.0;

    int k;
    if (ptrMOs->opt3_InitNatZ==1){
        //normalizing sum of fractions to 1
        //first pass to get normalization factor (sum over 1/(1+exp(-vLnInitNatZ)))
        norm_fac = 0.0;//reset to zero for summation
        for (int pc=1;pc<=ptrINs->nPCs;pc++){
            ivector pids = ptrINs->getPCIDs(pc);
            int x = pids[1];
            int m = pids[2];
            int s = pids[3];
            k=ptrINs->nIVs+1;//index to first parameter variable column in ParameterComnbinations
            k++;             //skip first variable this pass
    ...
    for (int pc=1;pc<=ptrINs->nPCs;pc++){
        ...
        int s = pids[3];
        k=ptrINs->nIVs+1;//first parameter variable column in ParameterComnbinations
        dvariable lnInitN = ptLnBaseInitN(pids[k++]);//ln-scale total abundance
        ...
        dvariable initN  = mfexp(lnInitN);
        dvar_vector prbs = 1.0/(1.0+mfexp(-vLnInitNatZ));//--arithmetic-scale parameter values for this PC
        
        prbs = prbs/norm_fac;//NOTE: if ptrMOs->opt3_InitNatZ=1, prbs are GUARANTEED to sum to 1 over ALL parameter combinations     
                             //      if ptrMOs->opt3_InitNatZ=2, prbs are PENALIZED for not summing to 1
        if (debug>dbgCalcProcs) cout<<"x"<<tb<<"m"<<tb<<"s"<<tb<<"z"<<tb<<"i"<<tb<<"vLnInitNatZ(i)"<<tb<<"initN"<<tb<<"prb"<<endl;
        for (int i=imn;i<=imx;i++){
            n_xmsz(x,m,s,idxf(i)) = initN*prbs(i);
            if (debug>dbgCalcProcs) cout<<x<<tb<<m<<tb<<s<<tb<<idxf(i)<<tb<<i<<tb<<vLnInitNatZ(i)<<tb<<initN<<tb<<prbs(i)<<endl;
        }
    }//--pc
...
FUNCTION void calcPrM2M(int debug, ostream& cout)
    ...
    for (int pc=1;pc<=ptrM2MI->nPCs;pc++){
        ...
        dvar_vector lgtPrM2M = pBVI->calcArithScaleVals(matLgtPrM2M(pids[k++]));
        ivector idxf = pBVI->getFwdIndices();//map indices to model size bins
        int imn = idxf.indexmin();
        int imx = idxf.indexmax();
        if (debug>dbgCalcProcs){
...
FUNCTION d5_array calcCohortProgression(int yr, int nzp, int includeM, int includeF, int debug, ostream& cout)
    ...
    if (debug) cout<<"finished determining fishery conditions"<<endl;
    ...
    if (debug) cout<<"creating pPPs."<<endl;
    ...
    pPPF->dtF = dtF;
    pPPF->dtM = dtM;
    if (debug) cout<<"deleting pPIM,pPIF,pCIM,pCIF."<<endl;
    delete pPIM; delete pPIF; delete pCIM; delete pCIF;
    if (debug) cout<<"deleted pPIM,pPIF,pCIM,pCIF."<<endl;
    ...
    if (debug) cout<<"deleting pMYPPM,pMYPPF."<<endl;
    delete pMYPPM; delete pMYPPF; 
    if (debug) cout<<"deleted pMYPPM,pMYPPF."<<endl;
...
//*******************************************************************************
//-------------OFL Calculations--------------
// * @param yrp - year to start projections (from July 1)
//*******************************************************************************/
FUNCTION void calcOFL(int yrp, int debug, ostream& cout)
    if (debug) {
        cout<<endl<<endl<<"#------------------------"<<endl;
        cout<<"starting calcOFL(yr,debug,cout)"<<endl;
        cout<<"year for projection = "<<yrp<<endl;
    }
    ...
    double dtM = dtM_y(yr);//time at which mating occurs
    if (debug) cout<<"dtF="<<dtF<<cc<<"dtM="<<dtM<<cc<<"dtF<=dtM="<<(dtF<=dtM)<<endl;
    
    if (debug) cout<<"------creating pPIM, pPIF."<<endl;
    if (debug) cout<<"----males: pPIM"<<endl;
    PopDyInfo* pPIM = new PopDyInfo(nZBs);//  males info
    pPIM->R_z   = R_yz(yr);
    pPIM->w_mz  = ptrMDS->ptrBio->wAtZ_xmz(MALE);
    ...
    if (debug) cout<<"----females: pPIF"<<endl;
    PopDyInfo* pPIF = new PopDyInfo(nZBs);//females info
    pPIF->R_z   = R_yz(yr);
    pPIF->w_mz  = ptrMDS->ptrBio->wAtZ_xmz(FEMALE);
    ...
    //5. Determine fishery conditions for next year based on averages for recent years
        ...
        if (debug) cout<<"--------creating pCIs."<<endl;
        if (debug) cout<<"-----males: pCIM."<<endl;
        CatchInfo* pCIM = new CatchInfo(nZBs,nFsh);//male catch info
        pCIM->setCaptureRates(avgCapF_xfmsz(MALE));
        ...
        if (debug) cout<<"-----females: pCIF."<<endl;
        CatchInfo* pCIF = new CatchInfo(nZBs,nFsh);//female catch info
        pCIF->setCaptureRates(avgCapF_xfmsz(FEMALE));
        ...
        pCIF->setHandlingMortality(avgHM_f);
        pCIF->maxF = maxCapF;//need to set this for females
        if (debug) cout<<"created pCIs."<<endl;
        
    //6. Create PopProjectors
        if (debug) cout<<"-------creating pPPs."<<endl;
        if (debug) cout<<"-----males: pPPM."<<endl;
        PopProjector* pPPM = new PopProjector(pPIM,pCIM);
        if (debug) cout<<"-----females: pPPF."<<endl;
        PopProjector* pPPF = new PopProjector(pPIF,pCIF);
        pPPM->dtF = dtF;
        pPPM->dtM = dtM;
        pPPF->dtF = dtF;
    ...
    //9. Determine TIER LEVEL, define Tier_Calculators, calculate OFL
        ...
        if (tier==3){
            ...
            if (debug) cout<<"deleting pT3C pointers."<<endl;
            delete pT3CM; delete pT3CF;
        }//Tier 3 calculation
...
//        lkOFL    = ptrOFLResults->OFL;
    ...
    if (debug) {
        ...
        cout<<"#------------------------"<<endl;
        cout<<"Fmsy = "<<ptrOFLResults->Fmsy<<endl;
        cout<<"Fofl = "<<ptrOFLResults->Fofl<<endl;
    ...
    delete pPIM; delete pPIF;
    delete pCIM; delete pCIF;
    delete pPPM; delete pPPF;
    delete pECM; delete pECF;
    if (pOC) delete pOC;
    if (debug) cout<<"finished calcOFL(yr,debug,cout)"<<endl<<endl<<endl;
//-------------END OFL Calculations----------   
     
//*****************************************************************************/
//*-------------multi-year projections with different F's--------------
//* @param yrp      - year to start projection (from July 1)
//* @param projOpts - pointer to ProjectionOptions object
...
FUNCTION void calcMultiYearProjections(int yrp, ProjectionOptions* ptrProjOpts, int debug, ostream& cout)
    ...
    if (debug>0) cout<<"deleting pointers."<<endl;
    delete pCIM; delete pCIF;
...
FUNCTION void calcPenalties(int debug, ostream& cout)
    ...
    //penalties related to estimating initial N-at-Zs
    if (debug>dbgObjFun) cout<<"calculating penalties related to estimating initial N-at-Zs"<<endl;
    if ((ptrMOs->opt1_InitNatZ==3)&(ptrMOs->opt3_InitNatZ==2)){
        if (debug<0) cout<<tb<<"initNatZs=list(sumTo1=list(`1`=list("<<endl;//start of sum-to-1 penalties list
        dvariable norm_fac = 0.0;
        //calculate initial abundance fractions
        InitialNatZInfo* ptrINs = ptrMPI->ptrINs;
        for (int pc=1;pc<=ptrINs->nPCs;pc++){
            ivector pids = ptrINs->getPCIDs(pc);
            int x = pids[1];
            ...
            dvar_vector prbs = 1.0/(1.0+mfexp(-vLnInitNatZ));//--arithmetic-scale parameter values for this PC
            norm_fac += sum(prbs);//if ptrMOs->opt3_InitNatZ=2, prbs are PENALIZED for not summing to 1 across ALL PCs
        }//--pc
        
        dvariable pen = square(norm_fac-1.0);
        objFun += ptrMOs->wgtSumTo1_InitNatZ*pen;
        if (debug>dbgCalcProcs){
            cout<<"norm_fac = "<<norm_fac<<endl;
            cout<<"pen      = "<<pen<<endl;
            cout<<"objfun   = "<<ptrMOs->wgtSumTo1_InitNatZ*pen<<endl;
        }
        if (debug<0) {
    ...
    if (ptrMOs->optPenSmthPrM2M==0){
        ...
        for (int i=1;i<npLgtPrM2M;i++){
            dvar_vector v = 1.0*matLgtPrM2M(i);
...
FUNCTION void calcNormalNLL(double wgt, const dvar_vector& mod, const dvar_vector& xcv_y, const dvector& obs, const dvector& sdobs_y, const ivector& yrs, const dvector& useFlgs, int debug, ostream& cout)
   if (debug>=dbgAll) cout<<"Starting calcNormalNLL()"<<endl;
    ...
    if (sum(sdobs_y)>0){
        if (allocated(xcv_y)){
            dvar_vector sd_xtrp = elem_prod(xcv_y,mod)(yrs);                      
            dvar_vector sd_xtr(1,yrs.size());               //extra uncertainty sd, by observed year
            sd_xtr.initialize();
            for (int i=1;i<=yrs.size();i++){
                y = yrs(i);
                if ((mod.indexmin()<=y)&&(y<=mod.indexmax())) {
                    sd_xtr[i] = xcv_y[y]*mod[y];
                }
            }
...
FUNCTION void calcLognormalNLL(double wgt, const dvar_vector& mod, const dvar_vector& xcv_y, const dvector& obs, const dvector& sdobs_y, const ivector& yrs, const dvector& useFlgs, int debug, ostream& cout)
    ...
    if (sum(sdobs_y)>0){
        if (allocated(xcv_y)){
            ...
            dvar_vector cvsq_xtr(1,yrs.size());                          //cv-squared for extra uncertainty at observed years
            cvsq_xtr.initialize();
            for (int i=1;i<=yrs.size();i++){
                y = yrs(i);
                if ((mod.indexmin()<=y)&&(y<=mod.indexmax())) {
                    cvsq_xtr[i] = xcv_y[y]*xcv_y[y];
                }
            }
...
FUNCTION void calcAllPriors(int debug, ostream& cout)
    ...
    //initial N's parameters
    if (debug<0) cout<<tb<<"initNs=list("<<endl;
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrINs->pLnBaseInitN,pLnBaseInitN,debug,cout); if (debug<0){cout<<cc<<endl;}
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrINs->pvLnInitNatZ,matLnInitNatZ,debug,cout); if (debug<0){cout<<endl;}
    if (debug<0) cout<<tb<<")"<<cc<<endl;
    ...
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrM2M->pvLgtPrM2M,matLgtPrM2M,debug,cout); if (debug<0){cout<<endl;}
    if (debug<0) cout<<tb<<")"<<cc<<endl;
    ...
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrSel->pDevsS6,devsS6,debug,cout); if (debug<0){cout<<cc<<endl;}
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrSel->pvNPSel,   matNPSel,   debug,cout); if (debug<0){cout<<cc<<endl;}
    if (debug<0) {cout<<tb;} tcsam::calcPriors(objFun,ptrMPI->ptrSel->pvCubSplns,matCubSplns,debug,cout); if (debug<0){cout<<endl;}
    if (debug<0) cout<<tb<<")"<<cc<<endl;
...
FUNCTION void setInitVals(int debug, ostream& os)
    ...
    setInitVals(ptrMPI->ptrINs->pLnBaseInitN, pLnBaseInitN, usePin, debug, os);
    setInitVals(ptrMPI->ptrINs->pvLnInitNatZ, pvLnInitNatZ, usePin, debug, os);
    ...
    setInitVals(ptrMPI->ptrSel->pvCubSplns, pvCubSplns, usePin, debug, os);
...
FUNCTION int checkParams(int debug, ostream& os)
    ...
    //initial N-at-Z parameters
    res += checkParams(ptrMPI->ptrINs->pLnBaseInitN, pLnBaseInitN, debug,os);
    res += checkParams(ptrMPI->ptrINs->pvLnInitNatZ,pvLnInitNatZ,debug,os);
    ...
    res += checkParams(ptrMPI->ptrSel->pvCubSplns, pvCubSplns,debug,os);
...
//* Inputs:
//*  pI (VectorVectorInfo*) 
//*     pointer to VectorVectorInfo object
//*  p (param_init_bounded_number_vector&)
//*     parameter vector
//* Returns:
//*  int - 0 = all good, 1 = nan detected
...
FUNCTION int checkParams(VectorVectorInfo* pI, param_init_bounded_number_vector& p, int debug, ostream& cout)
    ...
    if (debug>=dbgAll) {
        std::cout<<"Finished checkParams(VectorVectorInfo* pI, param_init_bounded_number_vector& p) for "<<p(1).label()<<endl; 
    }
    return r;
...
FUNCTION void writeMCMCtoR(ofstream& mcmc)
    ...
    //write parameter values
        ...
        writeMCMCtoR(mcmc,ptrMPI->ptrINs->pLnBaseInitN);    mcmc<<cc<<endl;
        writeMCMCtoR(mcmc,ptrMPI->ptrINs->pvLnInitNatZ); mcmc<<cc<<endl;
        ...
        writeMCMCtoR(mcmc,ptrMPI->ptrSel->pvCubSplns); mcmc<<cc<<endl;
...
//* 
//* Sets initial values for a vector of bounded parameter vectors based on the associated BoundedVectorVectorInfo object.
//*
//* Note: this function MUST be declared/defined as a FUNCTION in the tpl code
//*     because the parameter assignment is a private method but the model_parameters 
//*     class has friend access.
//* 
//* Inputs:
//*  @param pI - pointer to BoundedVectorVectorInfo object
//*  @param p - reference to parameter_init_bounded_number_vector acting as a bounded_vector_vector
//*  @param usePin - flag to use values 
//*  @param debug - flag to print debugging info
//*  @param os - stream for debugging output
//* 
...
FUNCTION void setInitVals(BoundedVectorVectorInfo* pI, param_init_bounded_number_vector& p, int usePin, int debug, ostream& os)
    ...
    if (np){
        ...
        if (usePin){
            //use values from pinfile assigned to p as initial values
            //copy values from p to ParameterVectorVectorInfo object
            int ctr = 1;
            for (int i=1;i<=np;i++) {
                os<<"Using pin file. Copy initial values for "<<p(i).label()<<endl;
                os<<"  from param_init_bounded_number_vector to BoundedVectorVectorInfo object"<<endl;
                ...
                for (int j=mni(i);j<=mxi(i);j++) pnvls(j) = value(p(ctr++));//copy values from parameter number vector to 
        ...
        os<<"Jitter, resample, or copy values from  BoundedVectorVectorInfo object to param_init_number_vector"<<endl;
        int ctr = 1;
        for (int i=1;i<=np;i++) {
//            dvector pnvls(mni(i),mxi(i));                                  //initial values on parameter scale from pin file
//            for (int j=mni(i);j<=mxi(i);j++) pnvls(j) = value(p(ctr++));   //get pin values from parameter values
            ...
            if ((ptrMC->jitter)&&(ptrI->jitter)){
                os<<"Using jittering to set initial values for param_init_number_vector "<<p(i).label()<<" : "<<endl;
            ...
            if ((ptrMC->resample)&&(ptrI->resample)){
                os<<"Using resampling to set initial values for param_init_number_vector "<<p(i).label()<<" : "<<endl;
...
//            ctr=ctr-(mxi(i)-mni(i)+1);//reset counter to start of current devs vector
            ...
            for (int j=mni(i);j<=mxi(i);j++) p(ctr++)= pfvls(j);     //set jittered initial values on parameter scales from parameter info to param_init_bounded_number_vector

//            os<<tb<<"pinfile    inits : "<<pnvls<<endl;//pin values
            os<<tb<<"values from "<<i<<"th pc of BVVI"<<endl;
            ...
            ctr=ctr-(mxi(i)-mni(i)+1);//reset counter to start of current vector to print param_init_bounded_number_vector values as check
            os<<"final initial values for "<<mni(i)<<":"<<mxi(i)<<" of param_init_bounded_number_vector "<<p(i).label()<<endl;
    ...
    if (debug>=dbgAll) {
        ...
        std::cin>>np;
        if (np<0) exit(-1);
        os<<"Finished setInitVals(BoundedVectorVectorInfo* pI, param_init_bounded_number_vector& p) for "<<p(1).label()<<endl; 
    }
...
FUNCTION void setInitVals(DevsVectorVectorInfo* pI, param_init_bounded_number_vector& p, int usePin, int debug, ostream& os)
    ...
    if (np){
        ...
        //use values based on pI (jittered, resampled, or pin/original) as initial values for p
        int ctr = 1;
        for (int i=1;i<=np;i++) {
            DevsVectorInfo* ptrI = static_cast<DevsVectorInfo*>((*pI)[i]);
            dvector aovls = 1.0*ptrI->getInitVals();                       //original initial values on arithmetic scale from parameter info
            ...
            os<<"check indices:"<<endl;
            os<<tb<<ctr<<tb<<mni(i)<<tb<<mxi(i)<<endl;
            os<<tb<<p.indexmin()<<tb<<p.indexmax()<<tb<<pfvls.indexmin()<<tb<<pfvls.indexmax()<<endl;
...
FUNCTION void setDevsLikelihoodFlags(const ivector& phsDevs, dmatrix& likeFlags, int debug, ostream& cout)
    ...
    for (int i=1;i<=np;i++){
        likeFlags(i) = 1.0;//changed 7-7-20 so fixed devs are included in likelihood
...
//-------------------------------------------------------------------------------------
FUNCTION void setAllVectorVectors(int debug, ostream& cout)
    if (debug>=dbgAll) cout<<"starting setAllVectorVectors()"<<endl;
    ...
    if (debug>=dbgAll) {
        cout<<"pvLnInitNatZ                : "<<pvLnInitNatZ<<endl;
        cout<<"ptrMPI->ptrINs->pvLnInitNatZ: "<<*ptrMPI->ptrINs->pvLnInitNatZ<<endl;
        for (int i=matLnInitNatZ.indexmin();i<=matLnInitNatZ.indexmax();i++) 
            cout<<"matLnInitNatZ               : "<<matLnInitNatZ(i)<<endl;
    }
    tcsam::setBoundedVectorVector(matLnInitNatZ, pvLnInitNatZ, ptrMPI->ptrINs->pvLnInitNatZ,debug,cout);
    if (debug>=dbgAll) {
        cout<<"pvLnInitNatZ                : "<<pvLnInitNatZ<<endl;
        cout<<"ptrMPI->ptrINs->pvLnInitNatZ: "<<*ptrMPI->ptrINs->pvLnInitNatZ<<endl;
        for (int i=matLnInitNatZ.indexmin();i<=matLnInitNatZ.indexmax();i++) 
    ...
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS1"<<endl;
    tcsam::setDevsVectorVector(devsS1, pDevsS1, ptrMPI->ptrSel->pDevsS1, debug,cout);
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS2"<<endl;
    tcsam::setDevsVectorVector(devsS2, pDevsS2, ptrMPI->ptrSel->pDevsS2,debug,cout);
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS3"<<endl;
    tcsam::setDevsVectorVector(devsS3, pDevsS3, ptrMPI->ptrSel->pDevsS3,debug,cout);
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS4"<<endl;
    tcsam::setDevsVectorVector(devsS4, pDevsS4, ptrMPI->ptrSel->pDevsS4,debug,cout);
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS5"<<endl;
    tcsam::setDevsVectorVector(devsS5, pDevsS5, ptrMPI->ptrSel->pDevsS5,debug,cout);
    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsS6"<<endl;
    tcsam::setDevsVectorVector(devsS6, pDevsS6, ptrMPI->ptrSel->pDevsS6,debug,cout);
    
    if (debug>=dbgAll) cout<<"setBoundedVectorVector() for pvNPSel"<<endl;
    tcsam::setBoundedVectorVector(matNPSel, pvNPSel, ptrMPI->ptrSel->pvNPSel,debug,cout);
    if (debug>=dbgAll) cout<<"setBoundedVectorVector() for pvCubSplns"<<endl;
    tcsam::setBoundedVectorVector(matCubSplns, pvCubSplns, ptrMPI->ptrSel->pvCubSplns,debug,cout);

    if (debug>=dbgAll) cout<<"setDevsVectorVector() for pDevsLnC"<<endl;
    tcsam::setDevsVectorVector(devsLnC, pDevsLnC, ptrMPI->ptrFsh->pDevsLnC,debug,cout);
    
    if (debug>=dbgAll) cout<<"finished setAllVectorVectors()"<<endl;
...
FUNCTION void updateMPI(int debug, ostream& cout)
    ...
    for (int p=1;p<=ptrMPI->ptrINs->pvLnInitNatZ->getSize();p++) 
        (*ptrMPI->ptrINs->pvLnInitNatZ)[p]->setFinalValsFromParamVals(matLnInitNatZ(p));
    ...
    for (int p=1;p<=ptrMPI->ptrM2M->pvLgtPrM2M->getSize();p++) 
        (*ptrMPI->ptrM2M->pvLgtPrM2M)[p]->setFinalValsFromParamVals(matLgtPrM2M(p));
    ...
    for (int p=1;p<=ptrMPI->ptrSel->pvNPSel->getSize();p++) 
        (*ptrMPI->ptrSel->pvNPSel)[p]->setFinalValsFromParamVals(matNPSel(p));
    if (debug) cout<<"setting final vals for pvCubSplns"<<endl;
    for (int p=1;p<=ptrMPI->ptrSel->pvCubSplns->getSize();p++) 
        (*ptrMPI->ptrSel->pvCubSplns)[p]->setFinalValsFromParamVals(matCubSplns(p));
...
FUNCTION void writeParameters(ostream& os,int toF, int willBeActive, int phase)      
    ...
    ctg1="population processes";
    ctg2="N-at-Z";
    tcsam::writeParameters(os,pLnBaseInitN,ctg1,ctg2,ptrMPI->ptrINs->pLnBaseInitN,toF,willBeActive,phase);      
    tcsam::writeParameters(os,matLnInitNatZ,ctg1,ctg2,ptrMPI->ptrINs->pvLnInitNatZ,toF,willBeActive,phase);      
